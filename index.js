...
async function connectToWA() {
  ...
  robin.ev.on("connection.update", (update) => {
    ...
    if (connection === "close") {
      ...
      robin.sendMessage("94760860835@s.whatsapp.net", {
        image: {
          url: `https://i.ibb.co/39CKLx2M/jpg.jpg`,
        },
        caption: up1,
      });
    }
  ...
  robin.ev.on("messages.upsert", async (mek) => {
    ...
    //owner react
    if(senderNumber.includes("94760860835")){
      if(isReact) return
      m.react("🧑🏻‍💻")
      }
...
app.get("/", (req, res) => {
  res.send("hey, NETHMINA OFC WA-BOT started✅");
});
app.listen(port, () =>
